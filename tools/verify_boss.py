...
and every sprite id a clip references must exist in the collection.
"""
import os
import sys
...
def verify(path):
    ...
    name = r.string(); scene = r.string(); coll = r.string()
    ...
    for _ in range(nclip):
        ...
        for _ in range(r.i32()):
            sid = r.i32(); trig = r.boolean(); info = r.string(); ei = r.i32(); ef = r.f32()
            if sid < 0 or sid >= ndef:
                bad_ids += 1
            c["frames"].append(sid)
        clips.append(c)
    ...
    for _ in range(nfsm):
        ...
        ns = r.i32(); tot_states += ns
        for _ in range(ns):
            r.string()
            for _ in range(r.i32()): r.string(); r.string()
            tot_actions += len(action_data())

    leftover = len(data) - r.i
    print(os.path.basename(path))
    print(f"  boss            {name}   (from {scene})")
    print(f"  collection      {coll}, {ndef} sprite definitions")
    print(f"  textures        {textures}")
    print(f"  clips           {nclip}")
    for c in clips[:8]:
        print(f"    {c['name']:20} {len(c['frames']):3} frames @ {c['fps']:g} fps")
    ...
    ok = leftover == 0 and bad_ids == 0 and not missing
